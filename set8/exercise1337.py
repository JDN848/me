...
def string_list_please() -> list:
    ...
    SameList = list_please()
    return SameList


def dictionary_please() -> dict:
    """Returns a dictionary, anything you like."""
    aDict = {"greeting": "hi", "name": "Jayden"}
    return aDict
...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    inputStringList = list(input_string.upper())
    fireStr = "🔥" + "🔥".join(inputStringList) + "🔥"
    return fireStr
...
def best_letter_for_pets() -> str:
    ...
    for letter in the_alphabet:
        ...
        if len(pet_list) > max_pet_count:
            max_pet_count = len(pet_list)
...
def random_filler_text(number_of_words=200) -> str:
    ...
    fillerDict = make_filler_text_dictionary()
    paragraphWords = []

    for i in range(number_of_words):
        wordLength = random.randint(3, 7)
        lengthyWords = fillerDict.get(wordLength, [])
        if lengthyWords:
            word = random.choice(lengthyWords)
            paragraphWords.append(word)

    paragraph = " ".join(paragraphWords)
    return paragraph
